...
# Trouble Shooting - 주소 기반 프로필 및 지역 판별 기능 개선
...
사용자 프로필에는 기존에 `region` 값을 직접 입력하도록 되어 있었다.

예를 들어 사용자가 `경기도 수원시`처럼 입력하거나, 정책 조건에는 `경기도`가 저장되어 있을 경우 지역 조건 판별이 명확하지 않았다.

또한 실제 서비스처럼 주소 검색을 통해 주소를 입력하는 방식이 아니어서 사용자 경험이 부족했고, 정책 지역 조건과 사용자 거주지를 비교하는 기준도 모호했다.

---

## 원인 분석

1. 사용자 프로필의 거주 정보가 단순 문자열 `region`으로 관리되고 있었다.
2. 사용자가 입력하는 주소 형식이 일정하지 않아 정책 조건과 정확히 비교하기 어려웠다.
3. 카카오 주소 검색 API를 사용하지 않아 실제 서비스 수준의 주소 입력 UX가 부족했다.
4. 주소 검색 결과는 `경기 수원시 ...`처럼 저장되지만, 정책 조건은 `경기도`처럼 저장되어 문자열 비교가 실패할 수 있었다.

---

## 해결 방법

1. `UserProfile`의 `region` 필드를 `address`로 변경하였다.
2. 마이페이지 프로필 화면에 카카오 주소 검색 API를 적용하였다.
3. 사용자가 주소 검색 버튼을 통해 주소를 선택하면 `address` 입력값에 자동 반영되도록 구현하였다.
4. 정책 조건은 기존처럼 `region` 조건을 사용하되, 판별 시에는 `profile.getAddress()`를 기준으로 비교하도록 수정하였다.
5. `경기`와 `경기도`처럼 표현이 다른 행정구역명을 비교할 수 있도록 `normalizeRegion()` 메서드를 추가하였다.
6. `compareAddress()`에서 주소와 정책 조건을 표준화한 뒤 포함 여부를 비교하도록 개선하였다.

---

## 결과

- 사용자는 직접 주소를 입력하지 않고 주소 검색 팝업을 통해 주소를 선택할 수 있게 되었다.
- 사용자 프로필에 실제 주소 정보가 저장되도록 개선되었다.
- 정책 조건의 `region` 값과 사용자 주소를 비교하여 지역 조건 판별이 가능해졌다.
- `경기 수원시 ...` 주소도 `경기도` 조건과 정상적으로 매칭될 수 있게 되었다.
- Eligibility Engine의 지역 판별 정확도와 사용자 경험이 함께 개선되었다.

---

## 배운 점

1. 실제 서비스에서는 주소처럼 형식이 중요한 데이터는 직접 입력보다 검색 API를 사용하는 것이 적절하다.
